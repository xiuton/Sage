...
- `--corpus <CORPUS>`：单文件语料路径（不传时默认 `corpus_cn.txt`）。
- `--corpus-dir <CORPUS_DIR>`：语料目录（递归收集 `.txt`）。
...
- `--sft-jsonl <SFT_JSONL>`：SFT 数据 jsonl 文件。
- `--sft-sample`：使用内置 prompt/response 样例数据。
- `--sft-sample-messages`：使用内置 messages 样例数据。
- `--sft-max-records <SFT_MAX_RECORDS>`：最多读取多少条 SFT 记录（默认 `0` 不限制）。
- `--artifact-dir <ARTIFACT_DIR>`：输出目录（默认 `./tmp/sage_model_formal`）。
- `--num-epochs <NUM_EPOCHS>`：训练 epoch（默认 `20`）。
- `--batch-size <BATCH_SIZE>`：batch size（默认 `16`）。
- `--lr <LR>`：学习率（默认 `0.0001`）。
- `--max-seq-len <MAX_SEQ_LEN>`：序列长度（默认 `64`）。
- `--force`：即使输出目录已有模型也强制重新训练/覆盖。
- `--continue`：从 `<artifact-dir>/model.mpk` 加载权重继续训练。
- `--resume-epoch <RESUME_EPOCH>`：从 `<artifact-dir>/checkpoint/model-<epoch>.mpk` 加载权重继续训练。
- `--reset-tokenizer`：忽略已有 `tokenizer.json`，从当前语料重新构建词表。
