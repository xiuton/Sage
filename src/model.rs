...
impl ModelConfig {
    pub fn init<B: Backend>(&self, device: &B::Device) -> Model<B> {
        ...
        let transformer_encoder = TransformerEncoderConfig::new(
            self.d_model,
            self.d_ff,
            self.n_heads,
            self.n_layers,
...
impl<B: Backend> Model<B> {
    pub fn forward(&self, input: Tensor<B, 2, Int>) -> Tensor<B, 3> {
        ...
        x = self.transformer_encoder.forward(TransformerEncoderInput::new(x));
    ...
    pub fn num_params(&self) -> usize {
        ...
        let mlp_params = (self.d_model * self.d_ff + self.d_ff) + (self.d_ff * self.d_model + self.d_model);
