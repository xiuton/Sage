...
- Chat 模式：`--chat`
  - 输入按 `<user>...</user>\n<assistant>...</assistant>` 模板组织
  - 输出提取 `<assistant>` 之后内容作为回复
- 交互模式：`--interactive`
- **停止序列**：
...
2. **更强的停止策略（Stop sequences）** ✅ **已完成**
   - 已实现：`--stop-on-user`（遇到 `<user>` 标签时停止）和 `--stop-sequence`（自定义停止序列）
   - 显著改善 chat 体验与输出长度可控性
3. **数据流式读取**（已完成）
...
- Stop sequences（遇到 EOS 或 `<user>` 停止）✅ **已完成**
- 更严格的 loss mask（真正只对 assistant 计算）
- 流式读取大 JSONL（不占满内存）✅ **已完成**

验收标准：

- `infer --chat` 输出不会无限续写到下一轮 `<user>`
